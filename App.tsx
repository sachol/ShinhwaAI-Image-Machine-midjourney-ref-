...
const App: React.FC = () => {
    ...
    const handleKeyInvalid = () => {
        ...
        setError('API 키가 유효하지 않습니다. 새 키를 입력해주세요.');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!apiKey) {
            setError("이미지를 생성하려면 먼저 API 키를 설정해야 합니다.");
            setIsApiKeyModalOpen(true);
            return;
        }
    ...
    const handleEditImage = async (editPrompt: string, uploadedImageBlob: Blob) => {
        if (!apiKey) {
            setError("이미지를 편집하려면 먼저 API 키를 설정해야 합니다.");
            setIsApiKeyModalOpen(true);
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-slate-100 font-sans text-slate-800 flex flex-col">
            <ApiKeyModal
                ...
                currentKey={apiKey}
            />
            <Header apiKey={apiKey} onApiKeyClick={() => setIsApiKeyModalOpen(true)} />
